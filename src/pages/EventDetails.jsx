...
export default function EventDetails() {
  ...
  const registration = hub.registrations.find(
    (item) =>
      item.userId === hub.profile.id &&
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-6">
          <p className="text-sm font-semibold text-cyan-700">
            {clubName(hub.clubs, event.clubId)} · {event.category}
          </p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <Info
              label="Seats"
              value={`${seatsLeft} of ${event.seatLimit} left`}
            />
          </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 text-center shadow-sm">
          {registration ? (
            ...
            <>
              ...
              <p className="mt-2 text-sm text-slate-500">
                One-click sign-up prevents duplicate entries and
